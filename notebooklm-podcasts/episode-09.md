# Episode 09: Color Choices And Structural Builds

Lesson range: 73-81

Episode focus: Choose lower-risk color paths and plan hard caps, shells, armatures, foam cores, wire, hardware, detachable pieces, and removal.

# Host Brief

Create a short 5-8 minute private study audio overview from this source.
...
## Lesson 73: Hard Cap Purpose

Objective: Understand rigid bases.

Core concept: A hard cap carries shapes that hair alone cannot hold. It must fit, breathe, and avoid sharp edges.

Practice prompt: Name one hairstyle that might justify a hard cap.

Check question: What comfort issue would you check first?

Ready when: You can explain why a shell might exist.
...
Objective: Test shape cheaply.

Core concept: A paper, tape, or scrap foam mockup can check size and shape before final materials.

Practice prompt: Describe a mini mockup for one large hair shape.

Check question: What question would the mockup answer?

Ready when: You can name one mockup goal.
...
Objective: Pick a hidden skeleton only when needed.

Core concept: An armature can hold shape, but it also adds complexity, weight, and safety concerns.

Practice prompt: Choose whether a floating ahoge needs no support, wire, or foam.

Check question: What carries the load?

Ready when: You can choose or reject armature support.
...
Objective: Hide lightweight bulk.

Core concept: Foam can make large shapes lighter than solid hair, but it must be covered convincingly.

Practice prompt: Name how you would cover one foam shape: wefts, fiber sheet, or surface hair.

Check question: Where might foam show?

Ready when: You can pair foam with a coverage method.
...
Objective: Use wire carefully.

Core concept: Wire can support shapes, but ends and pressure points must be protected from the wearer and the wig fiber.

Practice prompt: Write one safety rule for wire inside a wig.

Check question: Where could wire poke or bend badly?

Ready when: You can name one wire safety risk.
...
Objective: Use hardware when glue is not enough.

Core concept: Large headpieces often need clips, screws, snaps, straps, magnets, or other mechanical support.

Practice prompt: Pick one attachment method for horns, crown, or detachable hair mass.

Check question: Why is glue alone weak here?

Ready when: You can choose one mechanical method.
...
Objective: Plan removable hair pieces.

Core concept: A detachable section needs a secure connection, hidden join, repeatable placement mark, and packing plan.

Practice prompt: Name one hair piece that should detach for packing.

Check question: How would you hide the join?

Ready when: You can name one detachable build option.
...
Objective: Do not trap heat blindly.

Core concept: Rigid or dense builds need some comfort planning. Airflow and heat buildup matter before final finish.

Practice prompt: Name one place airflow could exist without ruining the look.

Check question: What would make the wig too hot?

Ready when: You can name one ventilation idea.
...
Objective: Plan how the wig comes off.

Core concept: If a wig uses adhesive, hardware, or a shell, the wearer needs a safe release order and reachable release points.

Practice prompt: Write one removal step for a complex wig.

Check question: Which release point must be reachable first?

Ready when: You can name a safe removal step.
